...
// /prefill?prefill_title=...&prefill_due=...&prefill_client_name=...
//
// Google Calendar 連携などから飛んできたとき、URLクエリの値を taskDraft
// (localStorage) に書き込んでから / にリダイレクトする中間ページ。
// 既存のタスク作成フォームの「下書き復元」機構をそのまま流用するため、
// メインページ側 (app/page.tsx) には変更を加えない構成。
function PrefillContent() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      // 顧客名 or ID から /api/clients で詳細を解決
